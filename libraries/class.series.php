...
class Series extends Hub {
	...
	function GetPastSchedule($Days) {
		$SchedulePrep = $this->PDO->prepare('SELECT Series.*, Episodes.* FROM Series, Episodes WHERE Episodes.SeriesKey = Series.SerieID AND EpisodeAirDate <= :CurrentTime AND EpisodeAirDate >= :DayOffset ORDER BY EpisodeAirDate DESC');
	...
	function GetFutureSchedule() {
		...
		JOIN (SELECT SeriesKey, MIN(EpisodeAirDate) MinDate FROM Episodes
		    WHERE EpisodeAirDate > :CurrentTime GROUP BY SeriesKey) M
